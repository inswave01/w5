...
var GridProto = {
  template: _.template(
    "<<%= tagName %> id='<%= id %>' class='gGrid<%= className %>' style='width:<%= width %>;" + "<%= style %>'>" +
      "<div class='gGrid-setTable'>" +
        ...
        "</div>" +
      "</div>" +
      "<div id='w5-dropdown-menu' class='w5-dropdown-menu <%= form_none %>'>" +
        "<ul role='menu'>" +
        "</ul>" +
      "</div>" +
    "</<%= tagName %>>"
  ...
  events: {
    ...
    "mouseenter .gGrid-table>thead>tr>th": function(e) { this.columnMenu.show.call( this, e ); },
    "mouseleave .gGrid-table>thead>tr>th": function(e) { this.columnMenu.hide.call( this, e ); }
  },
  keydownEvents: {
    38:  "moveUp",              // up
    40:  "moveDown",            // down
  ...
  initialize: function(options) {
    ...
    this.isBasic = _.isUndefined( this.group );
  ...
  render: function() {
    ...
    $el = $(this.template({
      ...
      form_none: this.isBasic ? 'form-none' : ''
    }));
    ...
    headerContextMenu = new ContextMenuView( {
      el: this.$('#w5-dropdown-menu'),
      grid: grid,
      placeholder: this.options.contextMenuOption ? this.options.contextMenuOption.datePlaceholder : ''
    } );
    this.addEvents();
    this.setResize();
  ...
  getColMenu: function ( col ) {
    ...
    var rightMenu = {
      _openColMenu : function( $menuBtn ) {
        var $cell = $menuBtn.closest("th"),
            frozenCol = grid.viewModel.getOption("frozenColumn"),
            i = frozenCol > 0 ? 0 : grid.startCol,
            colIdx = $cell.index(),
            targetCol = i + colIdx,
            left = 0,
            right,
            width,
            w1 = 14,  // button icon width
            w2 = 340, // rightMenu width
            w3 = 20;  // right padding
        ...
        for ( i; i <= targetCol; i++ ) {
          if ( i === frozenCol ) {
            i = grid.startCol;
            targetCol += ( i - frozenCol );
          }
          left += grid.viewModel.getMeta( ['*',i], 'width' );
        }
        left += grid.$wrapper_div.position().left;
        left -= w1;
        right = left + w2;
        ...
        $menuBtn.addClass("on");
        headerContextMenu.show( left, ( $cell.offset().top - grid.$el.offset().top ) + $cell.height(), colIdx, colID );
      },
      openColMenu : function( $menuBtn ){
        this._openColMenu( $menuBtn );
      },
      _closeColMenu : function ( $menuBtn ) {
        $menuBtn.removeClass("on");
        headerContextMenu.hide();
      },
      closeColMenu : function ( $menuBtn ) {
        this._closeColMenu($menuBtn);
      }
    };

    _(rightMenu).bindAll( "openColMenu", "closeColMenu" );

    $rightMenu.on( "click", function() {
      var $menuBtn = $(this).find(".w5-grid-colMenu-icon");
      if ( $menuBtn.hasClass("on") ) {
        rightMenu.closeColMenu( $menuBtn );
      } else{
        rightMenu.openColMenu( $menuBtn );
      }
    });  
...
//        frozenColumn = this.viewModel.getOption("frozenColumn"),
  ...
  getTargetCol: function (colIdx) {
    var frozenColumn = this.viewModel.getOption("frozenColumn");
    return colIdx < frozenColumn ? colIdx : colIdx - this.startCol + frozenColumn;
  },
  ...
  columnMenu: {
    show: function(e) {
      if ( !( this.$('#w5-dropdown-menu').hasClass('open') ) ) {
        $(e.target).closest('th').find('.display-right').removeClass('hide');
      }
    },
    hide: function(e) {
      if ( !( this.$('#w5-dropdown-menu').hasClass('open') ) ) {
        $(e.target).closest('th').find('.display-right').addClass('hide');
      }
    }
  }
};
